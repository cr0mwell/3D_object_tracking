...
// detects objects in an image using the YOLOv3 library and a set of pre-trained objects from the COCO dataset;
                   ...
                   std::string classesFile, std::string modelConfiguration, std::string modelWeights, bool bVis)
{
    ...
    cv::Size size = cv::Size(608, 608); // Default: 416*416
    ...
    if(bVis) {
        ...
        for(auto it=bBoxes.begin(); it!=bBoxes.end(); ++it) {
            ...
            label = classes[((*it).classID)] + ":" + label + ", id: " + to_string((*it).boxID);
